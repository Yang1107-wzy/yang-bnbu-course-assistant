...
- `v1.0.0` 已通过自动化 DOM 与页面函数测试，但在 2026 年 7 月 20 日正式窗口前尚未完成真实选课提交验收。
- 公开版只包含不会命中真实课程的虚构示例。首次使用必须打开 `设置`，填写自己的课程信息并保存。
...
3. Tampermonkey 显示安装页面后，确认名称为“Yang 抢课脚本”、版本为 `1.0.0`，再点击安装。
...
2. 打开面板的 `设置`。
3. 删除虚构示例，逐门填写：课程代码、MIS 页面显示的完整名称、四位班号、类别 `ME` 或 `FE`。
4. 保存后刷新页面，点击 `Test`。
...
确认网址是 `https://mis.bnbu.edu.cn/mis/student/es/elective.do` 或 `eleDetail.do`，脚本已启用，并强制刷新页面。
