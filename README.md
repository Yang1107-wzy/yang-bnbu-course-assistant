...
- `v1.2.0` 已通过自动化 DOM、页面函数、Worker 租约和面板布局测试，但在 2026 年 7 月 20 日正式窗口前尚未完成真实选课提交验收。
...
- 默认三门课程使用三个专用 Worker 并行扫描；更多目标由最多 6 个 Worker 均匀覆盖。
...
3. Tampermonkey 显示安装页面后，确认名称为“Yang 抢课脚本”、版本为 `1.2.0`，再点击安装。
...
4. 保存后刷新页面，点击 `Test`。
5. 控制中心显示三个 Worker 已打开后，再使用立即启动或预约启动。
...
点击 `立即启动` 后马上进入 `RUNNING`。三个专用 Worker 同时扫描：普通阶段每 3 秒刷新，开放前 30 秒至开放后 2 分钟每 1 秒刷新。发现 Select、Select from Waiting 或 Join Waiting List 后，该 Worker 立即冻结刷新并提交，不等待下一轮。
