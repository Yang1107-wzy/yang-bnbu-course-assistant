# Yang 抢课脚本 v1.2.3 使用与维护说明

> **仅供学习交流，禁止商业使用，不得用于学校正式选课、正式抢课、轮候或其他真实教务操作。请遵守中华人民共和国法律法规、学校规章及信息系统使用规定。** 本项目不是 BNBU 官方产品，不保证任何课程结果。

首次加载时必须完整阅读并勾选确认 `Yang-NCEL-1.0` 使用声明；确认前保持 `STOPPED`，只允许设置和 `Test`，不会创建自动 Worker 或提交动作。
...
| `src/compliance.js` | 许可证标识、声明版本、确认记录校验和统一合规文案 |
...
手动打开的详情页作为前台优先页，一次扫描本类别所有目标，并向 Worker 池发布类别健康心跳。后台最多只保留 `ME-1` 和 `FE-1`；已有健康前台页时不打开同类别 Worker。ME-1 扫描所有 ME 目标，FE-1 扫描所有 FE 目标。多门同时可选时进入全局优先 FIFO，动作间隔至少 250 ms。

Worker 打开租约为 15 秒；未成功认领时进入可见 `FAILED` 状态，按 5/15/30/60 秒退避重试，防止持续 OPENING 和标签风暴。
...
仓库默认预填 COMP3073 (1002)、COMP4213 (1001) 和 EBIS3113 (1002)。升级时，只有完全未经修改的旧三门公开默认配置或更早的三条 DEMO 配置会自动迁移；任何自定义目标都原样保留。其他使用者必须在面板的`设置`中填写自己的课程代码、完整名称、四位班号和 ME/FE 类别。
